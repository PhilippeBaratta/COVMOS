...
def discrete_assignment(cat,rho_itp,v_itp,non_0_,Nbr,cumu,les_randoms,rho,v_grid,Par,Ary):
    ...
    if Par['assign_scheme'] == 'trilinear':
        cat,rho_itp,v_itp = discrete_assignment_trilinear(cat,rho_itp,v_itp,non_0_,Nbr,cumu,les_randoms,rho,v_grid,Ary['grid_pos'],Ary['vertex'],N_sample,a,velocity,unit)
...
@njit(parallel=True,cache=True)
def discrete_assignment_trilinear(cat,rho_itp,v_itp,non_0_,Nbr,cumu,les_randoms,rho,v_grid,grid_pos,vertex,N_sample,a,velocity,unit):
    for i in prange(len(non_0_)):
        non_0         = non_0_[i]
        N_cell        = Nbr [non_0]
        cumu_non_0    = cumu[non_0]
